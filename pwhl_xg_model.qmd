...
Data for the model were pulled from the PWHL's API (using functions that I posted on [GitHub](https://github.com/ma-barbour/pwhl)). There are anomalies and errors in the data. The steps I took to "fix" the data are set out in painful detail below. Most people will have no interest in those details and can skip them. However, if you plan to use data from the PWHL's API then you need to know that you should not simply use the "raw" data.
...
This function converts the location data to a scale of 200:85 and places center ice at 0,0. After the adjustment, the axes represent distance in feet. This adjustment might not be appropriate in every case; however, it seems to produce reasonable results.
...
The metric used to evaluate all the models is area under the ROC curve, or roc_aur. For this metric, a larger number (closer to 1) is good.
...
The logistic regression models top the list, with the "interaction" model being the top performer. It should be acknowledged that none of these models has *great* performance.
...
Natalie Spooner really separated from the pack (of course, she had by far the most actual goals as well).
