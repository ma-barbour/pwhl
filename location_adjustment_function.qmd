...
## Adjust Shot Location And Add Shot Details
...
location_adjustment <- function(pbp_data) {
        ...
        find_sides <- pbp_data |>
                group_by(event_team) |>
                summarize(median_shot = round(median(x_location,
                                                     na.rm = TRUE), 1),
                          .groups = "drop") 
        ...
        pbp_data <- pbp_data |>
                mutate(distance = case_when(
                        median_shot > 0 & event == "shot" ~ round(abs(sqrt((x_location - 89)^2 + (y_location)^2)), 1),
                        median_shot < 0 & event == "shot" ~ round(abs(sqrt((x_location - (-89))^2 + (y_location)^2)), 1)),
                       .after = y_location)
        ...
        pbp_data <- pbp_data |>
                mutate(angle = case_when(
                        median_shot > 0 & event == "shot" ~ round(abs(atan((0-y_location) / (89-x_location)) * (180 / pi)), 1),
                        median_shot < 0 & event == "shot" ~ round(abs(atan((0-y_location) / (-89-x_location)) * (180 / pi)), 1)),
                       .after = distance) |>
                mutate(angle = ifelse((median_shot > 0 & x_location > 89) | (median_shot < 0 & x_location < -89), 180 - angle, angle))
